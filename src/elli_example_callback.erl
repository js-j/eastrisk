...
handle('GET', [<<"dial">>], Req) ->
    ...
    Num = elli_request:get_arg(<<"num">>, Req, <<"undefined">>),
    ast_manager:originate("Local/9000@localinst", "viva-outbound", Num, 1, 3000,
			  "", 1, []),
    {ok, [], <<"Dialed ", Num/binary>>};

handle('GET', [<<"dial">>,Num], _Req) ->
    %% Fetch a GET argument from the URL.
    %%    Name = elli_request:get_arg(<<"name">>, Req, <<"undefined">>),
    ast_manager:originate("Local/9000@localinst", "viva-outbound", binary_to_list(Num), 1, 3000, "", 1, []),
    {ok, [], <<"OK, calling ", Num/binary>>};
